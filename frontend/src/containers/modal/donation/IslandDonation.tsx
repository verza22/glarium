...
const IslandDonation: React.FC<IslandDonationProps> = ({ data }) => {
  const { t } = useTranslation();

  const getImage = () => {
    if (data.type === 1) return woodImg;
    ...
    <div className="py-3 box border rounded bg-yellow-100 text-center text-sm">
      ...
      {data.constructed_at === null ? (
        <div className="mt-3">
          ...
          <div className="mb-3 text-blue-500 cursor-pointer">{t("modal.donations.max")}</div>
          <div className="w-3/4 mx-auto py-1 bg-yellow-300 rounded cursor-pointer hover:bg-yellow-400">
